...
func TestInitialFen(t *testing.T) {
	v, turn := fenParser("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1")
	expected := [8][8]string{
	...
	if v != expected || turn != "w" {
		t.Error("Expected false, got ", v)
	}
}

func TestInGameFen(t *testing.T) {
	v, turn := fenParser("rnbqkbnr/pp1ppppp/8/2p5/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2")
	expected := [8][8]string{
		{BlackRook, BlackKnight, BlackBishop, BlackQueen, BlackKing, BlackBishop, BlackKnight, BlackRook},
